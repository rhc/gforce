namespace :util do
  def find_and_replace_in_source_files(find, replace)
    ...
    directories = %w[app config lib public script spec test db] # exclude bin, db, doc, log, and tmp
    directories.each do |d|
      files += Dir[File.join(d, "**/*.{rb,rake,haml,slim,erb,builder,js,coffee,css,scss}")]
    end
